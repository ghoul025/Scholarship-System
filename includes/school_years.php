...
if (!function_exists('enrollScholar')) {
    function enrollScholar($scholar_id, $school_year_id, $semester = '1st', $enrolled = 1, $notes = null) {
        $db = get_db();
        // Map semester to column in the new schema
        $col = $semester === '1st' ? 'enrolled_1st' : ($semester === '2nd' ? 'enrolled_2nd' : null);
        if (!$col) return false;

        // Check existing row for this scholar + school year
        ...
        try {
            if ($row) {
                // If flag already set to desired value, nothing to do
                $current = !empty($row[$col]) ? 1 : 0;
                $desired = $enrolled ? 1 : 0;
                if ($current === $desired) return true;

                // Determine the other semester column
                $other_col = $col === 'enrolled_1st' ? 'enrolled_2nd' : 'enrolled_1st';

                // Update the specific semester flag and, when enrolling, clear the other semester to enforce exclusivity
                ...
                // Insert a new row with the appropriate flags
...
if (!function_exists('getScholarEnrollment')) {
    function getScholarEnrollment($scholar_id, $school_year_id, $semester = null) {
        $db = get_db();
        // Return the single row for scholar + school year. Caller can inspect enrolled_1st/enrolled_2nd.
